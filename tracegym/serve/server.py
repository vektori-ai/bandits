"""HTTP transport for a reconstructed environment. Stdlib only.
    ...
    from tracegym.serve import SessionBackend, serve
...
tracegym has the server, which is the point: an RL environment that needs a
...
* ``/reset`` builds a new :class:`~tracegym.serve.backend.Episode` with its own
