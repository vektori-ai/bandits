...
in and hands one back) and leaves the tightening to :mod:`tracegym.state.entities`,
...
from tracegym.contracts import CallStatus, InvocationPoint, JsonObject, JsonValue, TraceCorpus
...
def find_identifiers(corpus: TraceCorpus) -> IdentifierIndex:
    ...
    **Evidence required.** At least one argument occurrence and one response
    ...
    :mod:`tracegym.state.entities`' job.
    ...
    **When evidence is insufficient.** A field seen only in arguments (e.g.
    ...
    :mod:`tracegym.state.entities` never lets an error body contribute fields.
    """
