...
in the returned :class:`~tracegym.contracts.StateSchema` is backed by an
...
from tracegym.contracts import EntitySchema, StateSchema, ToolSurface, TraceCorpus
...
def _is_static(draft: EntityDraft, writers: set[str], is_cross_referenced: bool) -> bool:
    ...
    **Evidence required.** To *avoid* being static, an entity needs either an
    ...
    evidence-gated (see :mod:`tracegym.state.relations`).
...
def infer_schema(corpus: TraceCorpus, surface: ToolSurface | None = None) -> StateSchema:
    """Infer the :class:`~tracegym.contracts.StateSchema` behind a corpus.
