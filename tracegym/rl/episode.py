...
thesis of tracegym is that reward is a state assertion rather than an opinion
...
:func:`tracegym.verify.run.evaluate` returns for the final store state and the
effect ledger -- 1.0 or 0.0 under the default all-or-nothing mode -- and then
:func:`tracegym.verify.anticheat.enforce` is applied, so a rollout that tripped
...
``UnsupportedToolError``
    ...
    ``tracegym/env/interface.py`` refuses under "failure discipline". So we do
...
guard 1 in :mod:`tracegym.verify.anticheat` able to notice a rollout that
...
from tracegym.env.session import TraceGymSession
from tracegym.verify import (
    AntiCheatReport,
    RolloutAction,
    RolloutRecord,
    check_rollout,
...
from tracegym.verify.run import UnreviewedVerifierError
...
class TraceEnv:
    ...
    schema, task, tool_classes, surface:
        ...
        :func:`tracegym.env.session.build_session`.
    verifier:
    ...
    reward_mode:
        Passed to :func:`tracegym.verify.run.evaluate`. Leave it
    ...
    def __init__(
        ...
        *,
        ...
        self.session: TraceGymSession | None = None
    ...
    def _build_session(self) -> TraceGymSession:
    ...
    def reset(self, seed: int | None = None) -> JsonObject:
        ...
        provenance, and is what :class:`~tracegym.rl.batch.TaskSuite` uses to
